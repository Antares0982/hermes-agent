...
def _locales_dir() -> Path:
    ...
    1. ``HERMES_LOCALES`` env var -- set by the Antares0982 Nix wrapper
       for backward compatibility.
    2. ``HERMES_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
       sealed-packaging system) to point at the installed catalog directory.
    3. ``<repo-root>/locales`` -- source checkouts and ``pip install -e .``,
    ...
    """
    # Backward compat: Antares0982 Nix wrapper sets HERMES_LOCALES
    override = os.getenv("HERMES_LOCALES", "").strip()
    if override:
